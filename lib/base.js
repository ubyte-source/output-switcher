(function (window) {
	...
	class Result {
		...
		constructor(switcher) {
			...
			this.getTable().getThead().drop().addRow(
				window.Table.Tr.Td.content(Dictionary.from()),
				window.Table.Tr.Td.content(Dictionary.to()),
				window.Table.Tr.Td.content(Dictionary.interval()),
				window.Table.Tr.Td.content(Dictionary.sample()),
				window.Table.Tr.Td.content(Dictionary.average()),
				window.Table.Tr.Td.content(Dictionary.saving())
			);
		}
		...
		update() {
			...
			tbody.drop();
			tbody.addRow(
				window.Table.Tr.Td.content(window.moment.unix(saving_date.from).utc().format(Info.format())),
				window.Table.Tr.Td.content(window.moment.unix(saving_date.to).utc().format(Info.format())),
				window.Table.Tr.Td.content(saving.getInterval().toString() + String.fromCharCode(32) + Um.time()),
				window.Table.Tr.Td.content(saving_sample.length.toString()),
				window.Table.Tr.Td.content(saving_sample_average.toFixed(1) + String.fromCharCode(32) + Um.original()),
				window.Table.Tr.Td.content(this.getSwitcher().getPercentage().toFixed(2) + String.fromCharCode(32) + Um.percentage())
			).getColumn(5).setRowspan(2);

			tbody.addRow(
				window.Table.Tr.Td.content(window.moment.unix(bypass_date.from).utc().format(Info.format())),
				window.Table.Tr.Td.content(window.moment.unix(bypass_date.to).utc().format(Info.format())),
				window.Table.Tr.Td.content(bypass.getInterval().toString() + String.fromCharCode(32) + Um.time()),
				window.Table.Tr.Td.content(bypass_sample.length.toString()),
				window.Table.Tr.Td.content(bypass_sample_average.toFixed(1) + String.fromCharCode(32) + Um.original())
			);
	...
	class Switcher {
				...
				yAxis: [
					{
						...
						labels: {
							...
							formatter: function () {
								return this.value.toString() + String.fromCharCode(32) + Um.voltage();
							},
						},
					},
					{
						...
						labels: {
							...
							formatter: function () {
								...
								return value.toString() + String.fromCharCode(32) + Um.power();
							},
						},
					},
				],
		...
		constructor(saving, bypass, select) {
			...
			this.saving = new Info(this);
			this.bypass = new Info(this);

			this.elements = {};
			this.elements.result = new Result(this);
		...
		getPercentage() {
			...
			return Info.percentage(saving.average(), bypass.average());
		}
		remap(index) {
			let output = [];
	...
	class Total {
		...
		constructor() {
			...
			this.getTable().getThead().addRow(
				window.Table.Tr.Td.content(Dictionary.total())
			).getColumn(0).setColspan(5);

			this.getTable().getThead().addRow(
				window.Table.Tr.Td.content(Report.progressive()),
				window.Table.Tr.Td.content(Report.time()),
				window.Table.Tr.Td.content(Report.saving()),
				window.Table.Tr.Td.content(Report.bypass()),
				window.Table.Tr.Td.content(Report.percentage())
			);
		}
		...
		addRow(date, saving, bypass) {
			let tbody = this.getTable().getTbody(),
				rows = tbody.getRows(),
				id = rows.length + 1,
				percentage = Info.percentage(saving, bypass);

			tbody.addRow(
				window.Table.Tr.Td.content(id),
				window.Table.Tr.Td.content(window.moment.unix(date).utc().format(Info.format())),
	...
	window.Switcher.option.Color = Um;
